...
Bruin supports Fluxx as a source for [Ingestr assets](/assets/ingestr), and you can use it to ingest data from Fluxx into your data platform.

To set up a Fluxx connection, you must add a configuration item in the `.bruin.yml` and `asset` file. You need `instance`, `client_id`, and `client_secret`. For details on how to obtain these credentials, please contact your Fluxx administrator to create an API client.

Follow the steps below to set up Fluxx correctly as a data source and run ingestion.
...
- `source_table`: The name of the data table in Fluxx that you want to ingest.
...
parameters:
  source_connection: my-fluxx
  source_table: 'grant_request:id,amount_requested,amount_recommended,granted'
  destination: postgres
```

## Available Source Tables

Fluxx source supports the following 50 tables:
